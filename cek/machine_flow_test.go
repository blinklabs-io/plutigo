...
func newTestMachineFlow() *Machine[syn.DeBruijn] {
	return NewMachineWithVersionCosts[syn.DeBruijn]([3]uint32{3, 0, 0}, 0)
}
...
func TestConstrCase(t *testing.T) {
	m := NewMachineWithVersionCosts[syn.DeBruijn]([3]uint32{3, 0, 0}, 0)
...
func TestDivisionByZeroBuiltin(t *testing.T) {
	...
	b := &Builtin[syn.DeBruijn]{Func: builtin.DivideInteger, Args: nil}
